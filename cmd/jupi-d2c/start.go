...
func newStartCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			return startDaemon(cmd.OutOrStdout())
		},
	}
	// 默认留空，真正的默认在 startDaemon 里按配置目录解析（见下）。
	cmd.Flags().StringVar(&logFile, "log-file", "", "后台进程日志文件路径（默认与配置同目录：./jupi-d2c.log 或 ~/.jupi-d2c/jupi-d2c.log）")
	return cmd
}

// startDaemon 在后台拉起服务并把启动信息写到 out。
// 供 start 与 restart 共用。
func startDaemon(out io.Writer) error {
	// 未显式指定 --log-file 时（restart 无此 flag，亦走此分支），
	// 把日志落点对齐到配置目录：开发（./config.yml）落工作目录，生产落 ~/.jupi-d2c。
	if logFile == "" {
	...
	fmt.Fprintf(out, "已启动 (pid %d)\n", pid)
	fmt.Fprintf(out, "  访问地址: http://localhost:%d\n", cfg.Port)
